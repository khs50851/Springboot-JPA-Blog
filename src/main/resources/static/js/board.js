let index = {
	init:function(){
		...
		$("#btn-delete").on("click",()=>{ // 이건 화살표 함수인데 function(){} 이렇게 안하는 이유는 this를 바인딩 하기 위해 function으로 사용하면 this값이 달라질 수 있음 펑션 사용할거면 let _this = this; 이렇게 먼저 바인딩 해놓고 아래에 _this.save() 이렇게 사용
			this.deleteById();
		});
	...
	// delete 함수 시작
	deleteById:function(){ 
		var id = $("#id").text(); // 아이디 값의 밸류값이 아닌 text를 뽑아야함
		$.ajax({
			type:"DELETE",
			url:"/api/board/"+id,
			dataType:"json" // 요청을 서버로 해서 응답이 왔을 때 기본적으로 버퍼로 오기때문에 모든 것이 String(문자열)임 근데 생긴게 json(json으로 지정하면)이라면 -> javascript오브젝트로 변경해줌
